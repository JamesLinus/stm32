...
int main(void)
{
    ...
    pthread_attr_setstacksize(&g_thread_attr[0], configMINIMAL_STACK_SIZE*10);
    pthread_setschedprio(&g_thread[0], tskIDLE_PRIORITY + 2UL);
    pthread_create(&g_thread[0], &g_thread_attr[0], Thread_Startup, 0);
    
    DEFINE_THREAD(Thread_DebugTX,     configMINIMAL_STACK_SIZE,     tskIDLE_PRIORITY + 1UL);
    DEFINE_THREAD(Thread_DebugRx,     configMINIMAL_STACK_SIZE*3, tskIDLE_PRIORITY + 1UL);
    DEFINE_THREAD(Thread_RFIntr,     configMINIMAL_STACK_SIZE*3, tskIDLE_PRIORITY + 3UL);
